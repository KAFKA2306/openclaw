...
const FACT_SIGNAL = /(?:\b(?:19|20)\d{2}\b|\b\d+(?:\.\d+)?%?\b|\b[0-9a-f]{7,40}\b|latest|current|version|release|commit|issue|pull request|最新|現在|正式|仕様|バージョン|コミット|イシュー|プルリク)/i;

function configuredAgentIds(pluginConfig) {
  const values = Array.isArray(pluginConfig?.agentIds) ? pluginConfig.agentIds : undefined;
  return values?.length ? new Set(values.filter((value) => typeof value === "string")) : DEFAULT_AGENT_IDS;
}
  ...
  register(api) {
    ...
    api.on(
      ...
      async (event, ctx) => {
        ...
        const text = typeof event.lastAssistantMessage === "string" ? event.lastAssistantMessage.trim() : "";
