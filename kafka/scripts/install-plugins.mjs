...
for (const plugin of plugins) {
  ...
  if (apply && !existsSync(entry)) {
    throw new Error(`missing built plugin entry: ${entry}`);
  }
...
exec("openclaw", [
  "config",
  "set",
  "plugins.entries.kafka-github-operations.enabled",
  "true",
  "--strict-json",
]);
exec("openclaw", [
  "config",
  "set",
  "plugins.entries.kafka-evidence-check.enabled",
  "true",
  "--strict-json",
]);
exec("openclaw", [
  "config",
  "set",
  "plugins.entries.kafka-evidence-check.hooks.allowConversationAccess",
  "true",
  "--strict-json",
]);
if (apply) {
...
console.log(
  apply
    ? "KAFKA plugins installed and policy config validated. Restart the Gateway."
    : "Dry run complete. Re-run with --apply to build, install, and configure plugins.",
);
